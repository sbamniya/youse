...
import type { SaveRelationshipInput, UnlinkRelationshipInput } from "./space.schema";
...
async function generateUniqueInviteCode(isUsed: (code: string) => Promise<boolean>) {
...
export const saveRelationship = async (userId: string, input: SaveRelationshipInput) => {
  const result = await prisma.$transaction(async (transaction) => {
    const existing = await transaction.userPartner.findFirst({
      where: { deletedAt: null, status: { in: ["invited", "accepted"] }, OR: [{ userId }, { partnerId: userId }] },
      orderBy: { invitedAt: "desc" },
    });
    const invitationCode = existing?.invitationCode ?? await generateUniqueInviteCode(
      async (code) => Boolean(await transaction.userPartner.findUnique({ where: { invitationCode: code }, select: { id: true } })),
    );
    ...
    if (existing) {
      const relationship = await transaction.userPartner.update({ where: { id: existing.id }, data: { ...relationshipData, invitedAt: existing.invitedAt ?? new Date() } });
      return { relationship, inviteCode: invitationCode, created: false };
    }
    const relationship = await transaction.userPartner.create({ data: { userId, ...relationshipData, invitedAt: new Date() } });
...
export const exportSpace = async (userId: string) => {
  ...
  return { exportedAt: new Date(), relationship: space, memories, plans, lists, checkIns, questions, moods, pokes };
};

export const unlinkRelationship = async (userId: string, input: UnlinkRelationshipInput) => {
  const space = await spaceFor(userId);
  const partnerId = space.userId === userId ? space.partnerId : space.userId;
  await prisma.$transaction(async (transaction) => {
    await transaction.user.update({ where: { id: userId }, data: { partnerId: null } });
    if (partnerId) await transaction.user.update({ where: { id: partnerId }, data: { partnerId: null } });
    if (input.mode === "delete") await transaction.userPartner.delete({ where: { id: space.id } });
...
export const reconnectRelationship = async (userId: string) => {
  const space = await prisma.userPartner.findFirst({
    where: { brokenAt: { not: null }, status: "accepted", OR: [{ userId }, { partnerId: userId }] },
  ...
  await prisma.reconnectRequest.upsert({
    where: { userPartnerId_requesterId: { userPartnerId: space.id, requesterId: userId } },
  ...
  if (!mutual) return { statusCode: 202, body: { mutual: false, message: "Request saved. Your partner will not be told unless they also ask." } };
  await prisma.$transaction(async (transaction) => {
  ...
  return { statusCode: 200, body: { mutual: true, message: "You are reconnected." } };
};
