...
export const list = async (req: Request, res: Response, next: NextFunction) => { try { res.json(await service.list(req.user!.id, req.query as { from: string; to: string })); } catch (error) { next(error); } };
