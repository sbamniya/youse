    ...
    <SafeAreaView style={{ flex: 1 }} className="bg-[#F4EFE7]" edges={['top', 'bottom']}>
      <View style={{ flex: 1 }} className="px-5 pt-3">
        ...
        <ScrollView style={{ flex: 1, minHeight: 1 }} showsVerticalScrollIndicator={false} contentContainerStyle={{ paddingTop: 35, paddingBottom: 110 }}>
          ...
          {step === 1 ? <View className="mt-9"><View className="mx-auto h-32 w-32 items-center justify-center overflow-hidden rounded-full bg-[#F3E2D1]"><Image source={{ uri: profileImage }} className="h-full w-full" /><View className="absolute bottom-1 right-1 rounded-full border border-[#D8D0C6] bg-[#F4EFE7] p-2"><Camera size={16} color="#18161B" /></View></View><Text className="mb-2 mt-8 text-[11px] uppercase tracking-[1px] text-[#716B74]">What’s your name?</Text><TextInput value={name} onChangeText={setName} className="h-14 rounded-xl border border-[#D8D0C6] bg-[#FFF9F2] px-4 text-[17px] text-[#18161B]" /></View> : null}
          ...
          {step === 6 ? <View className="mt-8"><Text className="mb-2 text-[11px] uppercase tracking-[1px] text-[#716B74]">Timezone</Text><View className="rounded-xl border border-[#D8D0C6] bg-[#FFF9F2] px-4 py-4"><Text className="text-[16px] text-[#18161B]">(GMT+5:30) India Standard Time</Text></View></View> : null}
...
function Choice({ label, detail, selected = false, onPress, grid = false }: { label: string; detail: string; selected?: boolean; onPress?: () => void; grid?: boolean }) {
  return <Pressable onPress={onPress} className={`${grid ? 'w-[48%] min-h-20' : ''} flex-row items-center justify-between rounded-xl border p-4 ${selected ? 'border-[#DF5B5F] bg-[#FBE0D8]' : 'border-[#D8D0C6] bg-[#FFF9F2]'}`}><View className="flex-1"><Text className="text-[12px] font-semibold text-[#18161B]">{label}</Text><Text className="mt-1 text-[10px] leading-4 text-[#716B74]">{detail}</Text></View>{selected ? <Check size={17} color="#DF5B5F" /> : <ChevronRight size={17} color="#9B949D" />}</Pressable>;
}
