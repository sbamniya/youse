      ...
      >
        <View>
          ...
          <View className="mt-1 border-t border-border-subtle px-4 pt-2">
            <View className="flex-row items-center justify-center">
              <View className="flex-1">
                <Text className="text-[72px] font-bold leading-[92px] tracking-[-4px] text-foreground">
                  82
                </Text>
              </View>
          ...
          <View className="mt-8 border-t border-border-subtle px-2 pt-7">
            <View className="flex-row items-center justify-between">
              ...
              <View className="flex-row items-center">
                ...
                <Text className="font-serif text-[14px] text-primary">
                  Combined mood
                </Text>
              </View>
            </View>
            ...
            <View className="mt-5">
              <View className="flex-row">
                <View
                  className="w-6 justify-between py-1"
                  style={{ height: 172 }}
                >
                  {[5, 4, 3, 2, 1].map((value) => (
                    <Text key={value} className="text-[14px] text-primary">
                      {value}
                    </Text>
                  ))}
                </View>
                <View className="flex-1">
                  <Svg height={172} width={chartWidth}>
                    ...
                    {moodData.map(({ label, value }, index) => {
                      const x =
                        18 +
                        (index * (chartWidth - 36)) / (moodData.length - 1);
                      const y = 18 + ((5 - value) / 4) * 144;
                  ...
                  <View className="mt-2 flex-row justify-between px-3">
                    {moodData.map(({ label }) => (
                      <Text key={label} className="text-[14px] text-primary">
                        {label}
                      </Text>
                    ))}
                  </View>
          ...
          <View className="mt-8 border-t border-border-subtle px-2 pt-7">
            ...
            <View className="mt-4">
              {weeklyActivity.map(
                ({ icon, label, progress, subtitle }, index) => (
                  <Pressable
                    key={label}
                  ...
                  >
                    <View className="h-10 w-10 items-center justify-center rounded-full bg-secondary">
                      <ThemedIcon icon={icon} size={18} strokeWidth={1.6} />
                    </View>
                    <View className="ml-4 flex-1">
                      <Text className="text-[16px] font-bold text-foreground">
                        {label}
                      </Text>
                      <Text className="mt-0.5 font-serif text-[14px] text-primary">
                        {subtitle}
                      </Text>
                    </View>
                    <Text className="text-[16px] font-bold text-foreground">
