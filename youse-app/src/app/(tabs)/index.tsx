...
export default function HomeScreen() {
  ...
  return (
    <SafeAreaView className="flex-1 bg-[#F4EFE7]" edges={['top']}>
      <ScrollView contentContainerStyle={{ paddingBottom: 28 }} showsVerticalScrollIndicator={false}>
        ...
        <View className="gap-2 px-3">
          ...
          <Pressable onPress={() => Alert.alert('Daily reflection', 'Your answer will be visible after both of you respond.')} className="rounded-2xl border border-[#D8D0C6] bg-[#EEE7DD] p-4">
          ...
          <View className="rounded-2xl bg-[#EEE7DD] p-4">
          ...
          <View className="flex-row gap-2">
            <View className="min-h-30 flex-1 rounded-2xl bg-[#EEE7DD] p-3">
          ...
          <Pressable onPress={() => Alert.alert('From Arjun', 'Shared a photo · 22m ago')} className="flex-row items-center rounded-2xl bg-[#EEE7DD] p-3">
          ...
          <View className="rounded-2xl bg-[#EEE7DD] p-4">
            ...
            <View className="mt-3 flex-row flex-wrap gap-2">
              {['Thinking of you', 'Proud of you', 'Miss you', 'Call me'].map((poke) => <Pressable key={poke} onPress={() => Alert.alert('Poke sent', poke)} className="w-[48%] rounded-xl bg-[#E5DDD2] px-2 py-3"><Text className="text-[10px] text-[#18161B]">{poke}</Text></Pressable>)}
            </View>
          </View>
          ...
          <View className="flex-row gap-2">
            {[['4', 'questions answered'], ['2', 'plans together'], ['3', 'shared moments']].map(([value, label]) => <View key={label} className="flex-1 rounded-2xl bg-[#EEE7DD] p-3"><Text className="font-serif text-[25px] text-[#18161B]">{value}</Text><Text className="mt-1 text-[9px] leading-3 text-[#716B74]">{label}</Text></View>)}
          </View>

          <View className="flex-row items-center overflow-hidden rounded-2xl bg-[#EEE7DD]"><Image source={{ uri: 'https://images.unsplash.com/photo-1522673607200-164d1b6ce486?auto=format&fit=crop&w=500&q=80' }} className="h-24 w-24" /><View className="flex-1 p-3"><Text className="text-[9px] uppercase tracking-[1px] text-[#716B74]">On this day</Text><Text className="mt-1 text-[11px] font-semibold text-[#18161B]">Your first Goa trip</Text><Text className="mt-1 text-[9px] text-[#716B74]">2 years ago · 8 photos</Text></View></View>
